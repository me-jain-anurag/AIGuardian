...
public:
  /// Construct with optional timeout (seconds), max concurrent sessions,
  /// and initial reserve capacity for action_sequence vectors.
  /// timeout_seconds = 0 means no timeout.
  /// max_sessions = 0 means unlimited.
  /// initial_reserve = pre-allocate capacity per session (default 64).
  explicit SessionManager(uint32_t timeout_seconds = 0,
                          uint32_t max_sessions = 0,
                          uint32_t initial_reserve = 64);
