...
std::string Config::to_json() const {
  ...
  // Sandbox
  j["sandbox"] = {{"memory_limit_mb", sandbox.memory_limit_mb},
  ...
  // Policy
  j["policy"] = {{"default_policy_path", policy.default_policy_path},
...
Config Config::from_json(const std::string &json_str) {
  ...
  if (j.contains("cycle_detection")) {
    ...
    if (cd.contains("default_threshold") &&
        cd["default_threshold"].is_number_unsigned()) {
      cfg.cycle_detection.default_threshold =
          cd["default_threshold"].get<uint32_t>();
    }
    if (cd.contains("per_tool_thresholds") &&
        cd["per_tool_thresholds"].is_object()) {
      cfg.cycle_detection.per_tool_thresholds =
    ...
    if (sb.contains("timeout_ms") && sb["timeout_ms"].is_number()) {
      ...
      if (val >= 100 && val <= 60000) {
        ...
        std::cerr << "[WARN] Config: timeout_ms " << val
                  << " out of range (100-60000), using default 5000"
                  << std::endl;
      }
    }
    if (sb.contains("allowed_paths") && sb["allowed_paths"].is_array()) {
      cfg.sandbox.allowed_paths =
          sb["allowed_paths"].get<std::vector<std::string>>();
    }
  ...
  if (j.contains("performance")) {
    ...
    if (perf.contains("cache_size") &&
        perf["cache_size"].is_number_unsigned()) {
      cfg.performance.cache_size = perf["cache_size"].get<uint32_t>();
    }
    if (perf.contains("enable_string_interning") &&
        perf["enable_string_interning"].is_boolean()) {
      cfg.performance.enable_string_interning =
          perf["enable_string_interning"].get<bool>();
    }
    if (perf.contains("wasm_module_caching") &&
        perf["wasm_module_caching"].is_boolean()) {
      cfg.performance.wasm_module_caching =
          perf["wasm_module_caching"].get<bool>();
    }
    if (perf.contains("wasm_cache_size") &&
        perf["wasm_cache_size"].is_number_unsigned()) {
  ...
  if (j.contains("policy")) {
    auto &pol = j["policy"];
    if (pol.contains("default_policy_path") &&
        pol["default_policy_path"].is_string()) {
      cfg.policy.default_policy_path =
          pol["default_policy_path"].get<std::string>();
    }
