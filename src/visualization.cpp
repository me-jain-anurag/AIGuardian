...
std::string
VisualizationEngine::render_graph(const PolicyGraph &graph,
  ...
  if (options.output_format == VisualizationOptions::SVG ||
    ...
    std::string fmt =
        options.output_format == VisualizationOptions::SVG ? "svg" : "png";
    std::string cmd =
        "dot -T" + fmt + " " + tmp_dot.string() + " -o " + tmp_out.string();
...
std::string VisualizationEngine::render_sequence(
    const PolicyGraph &graph, const std::vector<ToolCall> &sequence,
  ...
  for (size_t i = 0; i < sequence.size(); ++i) {
    ...
    std::string base_color =
        node_opt ? node_dot_color(node_opt->node_type) : "lightgray";
  ...
  for (size_t i = 0; i + 1 < sequence.size(); ++i) {
    bool next_approved =
        (i + 1 < results.size()) ? results[i + 1].approved : true;
...
std::string
VisualizationEngine::render_ascii_graph(const PolicyGraph &graph) const {
  ...
  out << "\n"
      << ANSI_BOLD << "  Legend:\n"
      << ANSI_RESET << "  " << ANSI_RED << "■" << ANSI_RESET
      << " SENSITIVE_SOURCE    "
...
std::string VisualizationEngine::render_ascii_sequence(
  ...
  for (size_t i = 0; i < sequence.size(); ++i) {
    ...
    const char *node_color =
        node_opt ? node_ansi_color(node_opt->node_type) : ANSI_RESET;

    // Step header
    out << "  Step " << (i + 1) << ": " << node_color << ANSI_BOLD
        << call.tool_name << ANSI_RESET;
    ...
    if (approved) {
      ...
      if (i < results.size() && results[i].detected_exfiltration) {
        ...
        out << "\n  " << ANSI_RED << "    ⚠ Exfiltration: " << exfil.source_node
  ...
  out << "\n"
      << ANSI_BOLD << "  ──────────────────────────────\n"
      << "  Summary: " << ANSI_GREEN << approved_count << " approved  "
      << ANSI_RED << blocked_count << " blocked" << ANSI_RESET << "\n\n";
