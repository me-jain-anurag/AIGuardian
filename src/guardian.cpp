...
Guardian::Guardian(const std::string &policy_file_path,
  ...
  if (!fs::exists(policy_path)) {
    throw std::runtime_error("Guardian: policy file not found: " +
                             policy_file_path);
  }

  std::ifstream file(policy_path);
  if (!file.is_open()) {
    throw std::runtime_error("Guardian: could not open policy file: " +
                             policy_file_path);
  }
  ...
  // Use default (mock) factory — in production, replace with WasmEdgeRuntime
  // factory
  ...
  sandbox_mgr_ =
      std::make_unique<SandboxManager>(wasm_tools_dir, default_factory);
  if (!wasm_tools_dir.empty() && fs::exists(wasm_tools_dir)) {
    ...
    for (const auto &entry : fs::directory_iterator(wasm_tools_dir)) {
      if (entry.path().extension() == ".wasm") {
        sandbox_mgr_->load_module(entry.path().stem().string(),
                                  entry.path().string());
      }
    }
  }
...
std::string Guardian::visualize_session(const std::string &session_id,
  ...
  return viz_->render_sequence(policy_graph_, sequence,
                               std::vector<ValidationResult>{}, opts);
}
...
Config Guardian::get_config() const { return config_; }

// ============================================================================
// Accessors
// ============================================================================

const PolicyGraph &Guardian::get_policy_graph() const { return policy_graph_; }

bool Guardian::is_initialized() const { return initialized_; }
