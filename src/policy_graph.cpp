...
static std::string node_type_to_string(NodeType type) {
    switch (type) {
        ...
        case NodeType::SENSITIVE_SOURCE:     return "sensitive_source";
        case NodeType::EXTERNAL_DESTINATION: return "external_destination";
        case NodeType::DATA_PROCESSOR:       return "data_processor";
        default:                             return "normal";
    }
}

static NodeType string_to_node_type(const std::string& s) {
    if (s == "sensitive_source")     return NodeType::SENSITIVE_SOURCE;
    if (s == "external_destination") return NodeType::EXTERNAL_DESTINATION;
    if (s == "data_processor")       return NodeType::DATA_PROCESSOR;
...
static SandboxConfig json_to_sandbox_config(const json& j) {
    SandboxConfig cfg;
    if (j.contains("memory_limit_mb"))  cfg.memory_limit_mb = j["memory_limit_mb"].get<uint64_t>();
    if (j.contains("timeout_ms"))       cfg.timeout_ms = j["timeout_ms"].get<uint32_t>();
    if (j.contains("allowed_paths"))    cfg.allowed_paths = j["allowed_paths"].get<std::vector<std::string>>();
    if (j.contains("network_access"))   cfg.network_access = j["network_access"].get<bool>();
    if (j.contains("environment_vars")) cfg.environment_vars = j["environment_vars"].get<std::map<std::string, std::string>>();
    return cfg;
}
...
PolicyGraph PolicyGraph::from_json(const std::string& json_str) {
    ...
    for (const auto& node_obj : j["nodes"]) {
        ...
        if (node_obj.contains("tool_name"))    node.tool_name = node_obj["tool_name"].get<std::string>();
        if (node_obj.contains("risk_level"))   node.risk_level = string_to_risk_level(node_obj["risk_level"].get<std::string>());
        if (node_obj.contains("node_type"))    node.node_type = string_to_node_type(node_obj["node_type"].get<std::string>());
        if (node_obj.contains("wasm_module"))  node.wasm_module = node_obj["wasm_module"].get<std::string>();
        if (node_obj.contains("metadata"))     node.metadata = node_obj["metadata"].get<std::map<std::string, std::string>>();
    ...
    if (j.contains("edges") && j["edges"].is_array()) {
        for (const auto& edge_obj : j["edges"]) {
            ...
            if (edge_obj.contains("conditions")) {
                edge.conditions = edge_obj["conditions"].get<std::map<std::string, std::string>>();
            }
            if (edge_obj.contains("metadata")) {
                edge.metadata = edge_obj["metadata"].get<std::map<std::string, std::string>>();
            }
            graph.add_edge(edge);
        }
    }
...
static std::string dot_color_for_node_type(NodeType type) {
    switch (type) {
        ...
        case NodeType::DATA_PROCESSOR:       return "green";
...
static std::string dot_shape_for_risk(RiskLevel level) {
    switch (level) {
        case RiskLevel::CRITICAL: return "doubleoctagon";
        case RiskLevel::HIGH:     return "octagon";
        case RiskLevel::MEDIUM:   return "hexagon";
        default:                  return "box";
    }
}
...
PolicyGraph PolicyGraph::from_dot(const std::string& dot_str) {
    ...
    while (std::getline(stream, line)) {
        ...
        if (std::regex_search(line, match, node_regex)) {
            ...
            if (std::regex_search(attrs, label_match, label_regex)) {
                ...
                node.tool_name = (nl_pos != std::string::npos)
                    ? label.substr(0, nl_pos) : label;

                // Extract node_type from label parenthetical
                if (nl_pos != std::string::npos) {
                    auto type_str = label.substr(nl_pos + 2);
                    // Remove parentheses
                    if (type_str.front() == '(') type_str = type_str.substr(1);
                    if (type_str.back() == ')')  type_str.pop_back();
            ...
            if (std::regex_search(attrs, fill_match, fill_regex)) {
                ...
                if (color == "red")        node.node_type = NodeType::SENSITIVE_SOURCE;
                else if (color == "orange") node.node_type = NodeType::EXTERNAL_DESTINATION;
                else if (color == "green")  node.node_type = NodeType::DATA_PROCESSOR;
            }
            ...
            if (std::regex_search(attrs, shape_match, shape_regex)) {
                ...
                if (shape == "doubleoctagon") node.risk_level = RiskLevel::CRITICAL;
                else if (shape == "octagon")  node.risk_level = RiskLevel::HIGH;
                else if (shape == "hexagon")  node.risk_level = RiskLevel::MEDIUM;
            }
    ...
    while (std::getline(stream, line)) {
        ...
        if (std::regex_search(line, match, edge_regex)) {
            ...
            if (graph.has_node(edge.from_node_id) && graph.has_node(edge.to_node_id)) {
                graph.add_edge(edge);
            }
        }
    }
